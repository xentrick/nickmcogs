...
class ServerStats(commands.GroupCog):
    ...
    async def guild_embed(self, guild: discord.Guild) -> discord.Embed:
        ...
        # Without the presence intent every member reads as offline with no
        # activities. Since zero-count rows are skipped below, leaving these in
        # would render a single white circle carrying the entire member count --
        # i.e. "everyone is offline" stated as fact. Omit the rows instead, and
        # read the intent live so they return if it is ever re-granted.
