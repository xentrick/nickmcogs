...
class Leveler(commands.Cog):
    ...
    def make_full_profile(self, avatar_data, user, xp, nxp, lvl, minone, elo, ldb, desc, bg=None):
        ...
        font3 = ImageFont.truetype(rsc_font, 32)
        ...
        for line in textwrap.wrap(desc, width=20):
            count += 1
            if count == 6:
                draw.text((margin, offset), f"{line}...", fill=desc_color, font=font1)
                break
            draw.text((margin, offset), f"{line}", fill=desc_color, font=font1)
    ...
    async def profile_data(self, user):
        ...
        if not await self.profiles._is_registered(user):
            ...
            log.debug(f"User Guild: {user.guild}")
            log.debug(f"Profile Roles: {roles}")
            if len(roles) == 0:
                default = await self.profiles.data.guild(user.guild).defaultrole()
                ...
                if str(lvl) in roles.keys():
                    ...
                    if tmp == 0:
                        ...
                        log.debug(f"Roles: {user.guild.roles}")
    ...
    async def show(self, ctx):
        ...
        log.debug(f"CTX Guild: {ctx.guild}")
        log.debug(f"Showing Roles: {roles}")
        if len(roles) == 0:
            await ctx.send(_("No roles yet configured for this guild !"))
            return
        for k, v in roles.items():
            try:
                rl = discord.utils.get(ctx.guild.roles, id=v)
                emb.add_field(name=str(k), value=rl.name)
                tmp += 1
                if tmp == 25:
                    emblist.append(emb)
                    emb = discord.Embed()
