...
class OnlineStats(commands.Cog):
	...
	def presence_available(self):
		"""Whether the privileged presence intent was granted.

		Without it every member reports offline on every device and
		`Member.activities` is always empty, so both commands in this cog would
		return a confident wrong answer rather than no answer. Read live so they
		start working again by themselves if the intent is re-enabled.
		"""
	...
	async def onlinestatus(self, ctx):
		...
		if not self.presence_available():
			await ctx.send(
				'This bot was not granted the presence intent, so device status '
				'is not visible. Every member would be counted as offline, so '
	...
	async def onlineinfo(self, ctx, *, member: discord.Member=None):
		...
		if not self.presence_available():
			await ctx.send(
				'This bot was not granted the presence intent, so device status '
				'is not visible. Every device would show as offline.'
			)
			return
		if member is None:
			member = ctx.author
